...
class UIManager {
    ...
    static initialize() {
        ...
        this.outerEditContainer = document.getElementById('appointment-edit-container');
...
static setupActionButtons(appointmentId) {
   ...
   static async editAppointment(appointmentId) {
    try {
        if (!appointmentId) {
            console.log('❌ ID نامعتبر برای editAppointment');
            return;
        }

        const appointment = await AppointmentAPI.getAppointmentDetails(appointmentId);
        const outerEditContainer = document.getElementById('appointment-edit-container');
        ...
        if (outerEditContainer) outerEditContainer.style.display = 'block';
        if (viewContainer) viewContainer.style.display = 'none';
        if (editContainer) editContainer.style.display = 'block';

        // پر کردن فرم با اطلاعات نوبت
        UIManager.fillEditForm(appointment);

        // راه‌اندازی date و time picker
        UIManager.setupDateTimePickers();

        // اتصال event submit فقط یک‌بار
        const form = document.getElementById('appointmentEditForm');
        if (form && !form.dataset.listenerAttached) {
            form.addEventListener('submit', UIManager.handleFormSubmit);
            form.dataset.listenerAttached = "true";
            console.log("✅ فرم ویرایش آماده و listener متصل شد.");
        }

        console.log("🧪 edit mode is active");

    } catch (error) {
        console.error('خطا در بارگذاری فرم ویرایش:', error);
        UIManager.showNotification('خطا در بارگذاری اطلاعات نوبت', 'error');
    }
}
     ...
     static fillEditForm(appointment) {
        ...
        const fields = {
         ...
         const timeFields = {
             ...
             'time_from': appointment.time_from?.substring(0, 5),
             'time_to': appointment.time_to?.substring(0, 5),
         };
        ...
        const createdAtInput = form.querySelector('[data-role="created"]');
        const doctorInput = form.querySelector('[name="treating_doctor"]');
        if (doctorInput) doctorInput.value = appointment.treating_doctor?.id || '';
        // تنظیم چک‌باکس‌ها
        ...
        if (noFileCheckbox) noFileCheckbox.checked = appointment.no_file;
         if (createdAtInput) {
             ...
             createdAtInput.value = createdAt.toLocaleDateString('fa-IR');
         }
    }
    ...
    static addEventListeners() {
        const form = document.getElementById('appointmentEditForm');
            if (form) {
            console.log("✅ فرم پیدا شد و رویداد متصل شد");
            form.addEventListener('submit', UIManager.handleFormSubmit);
        } else {
        console.warn("⚠️ فرم 'appointmentEditForm' پیدا نشد!");
    }
            ...
            searchInput.addEventListener('input', (e) => {
                ...
                if (query.length >= 2) {
                    searchTimeout = setTimeout(async () => {
                        const result = await AppointmentAPI.searchPatient(query);
                            if (result.status === 'success') {
                                UIManager.showSearchResults(result.results);
                            } else {
                                UIManager.showNotification(result.message || 'خطا در جستجوی بیمار', 'error');
                            }
                        }, 500);
                    }
                });
            }
// نوع درمان و شرح درمان (Dropdown dependent)
            const treatmentType = document.getElementById('visit_treatment_type');
            const treatmentDescription = document.getElementById('visit_treatment_description');

                if (treatmentType && treatmentDescription) {
    // هنگام لود: دریافت لیست نوع درمان
                    AppointmentAPI.getTreatmentTypes().then(res => {
                    if (res.status === 'success') {
                        treatmentType.innerHTML = '<option value="">  </option>';
                            res.results.forEach(type => {
                            const opt = document.createElement('option');
                            opt.value = type.id;
                            opt.textContent = type.title;
                            treatmentType.appendChild(opt);       
                            });
                    } else {
                        treatmentType.innerHTML = `<option value="">${res.message}</option>`;
                    }
                            });

                    // واکنش به انتخاب نوع درمان: لود شرح درمان
                            treatmentType.addEventListener('change', async function () {
                            const typeId = treatmentType.value;
                            treatmentDescription.innerHTML = '<option value="">در حال بارگذاری...</option>';
                        if (!typeId) {
                            treatmentDescription.innerHTML = '<option value="">ابتدا نوع درمان را انتخاب کنید</option>';
                            return;
                    }

                    const res = await AppointmentAPI.getTreatmentDetailsByType(typeId);
                    if (res.status === 'success') {
                        treatmentDescription.innerHTML = '<option value="">شرح درمان را انتخاب کنید</option>';
                        res.results.forEach(detail => {
                        const opt = document.createElement('option');
                        opt.value = detail.description;
                        opt.textContent = `${detail.description} (${detail.international_code || 'بدون کد'})`;
                        treatmentDescription.appendChild(opt);
                });
                    } else {
                    treatmentDescription.innerHTML = `<option value="">${res.message}</option>`;
                    }
            });
    }
}
  ...
  static async handleDelete() {
   ...
   static calculateEndTime(startTime, duration) {
    ...
    container.innerHTML = '';
    if (results.length === 0) {
        container.innerHTML = '<div class="search-result-item empty">موردی یافت نشد</div>';
    ...
    const fields = {
        ...
        'visit_file_number': patient.file_num || ''
    };
...
static async displayAppointmentDetails(appointment_id) {
   ...
   static async enterEditMode(appointment_id) {
    ...
    // پاک کردن حالت‌ها و داده‌ها
    delete this.modal.dataset.appointmentId;
    delete this.modal.dataset.editMode;

    this.clearAllFieldErrors(); // پاک کردن ارورهای فرم

    // فرم جدید اگر فعال بود
    if (this.form && this.form.style.display !== 'none') {
        this.form.reset();
        this.form.style.display = 'none';
    }

    // فرم ویرایش اگر فعال بود
    if (this.editForm && this.editForm.style.display !== 'none') {
        this.editForm.reset?.(); // اگر داشته باشه
        this.editForm.style.display = 'none';
    }

    // جزئیات نوبت (ویو فقط خواندنی)
    const viewContainer = this.modal.querySelector('.appointment-view-container');
    if (viewContainer) {
        viewContainer.remove();
    }

    // مخفی کردن بخش‌های بیرونی مربوط به ویرایش
    this.viewContainer?.style && (this.viewContainer.style.display = 'none');
    this.outerEditContainer?.style && (this.outerEditContainer.style.display = 'none');
}
    ...
    const data = {
        ...
        // فیلدهای جدید
        treatment_type: document.getElementById('visit_treatment_type')?.textContent,
...
static getEditFormData() {
    ...
    if (!form) {
        console.warn("🛑 فرم ویرایش پیدا نشد");
        return data;
    }
...
static async handleFormSubmit(event) {
    ...
    try {
        ...
        const editable_data = UIManager.getEditFormData();
        // بررسی آیا ویرایش است یا ثبت جدید
        const appointmentId = form.dataset.appointmentId;
        let response;
        if (appointmentId) {
            // ویرایش نوبت موجود
            // این تابع برای فرم ویرایش فقط از API استفاده می‌کنه و تابع داخلی نداره
            response = await AppointmentAPI.updateAppointment(appointmentId, editable_data);
            UIManager.showNotification('نوبت با موفقیت  به‌روزرسانی شدپیام :هندلر :', 'success');
            console.log('📌 handleFormSubmit connected');
        } else {
            // ثبت نوبت جدید
